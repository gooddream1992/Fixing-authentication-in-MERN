...
import {doubte_update, chatuser_add_room, other_admin, doubte_all_update} from 'Function/SupportChat.js';
...
export default function TableList(props) {
  ...
  const ChatOpen = (data) => {
    window.location.href="/admin/support_chat/"+data.id+"/"+userData._id;
  }
  ...
  function sendMessage(item) {
    var data = {
      ...
      userId: item._id,
      user_name: item.first_name+" "+item.last_name,
      avatarURL: server_url+"admin/user/img_get/"+item.image
    }
    chatuser_add_room(data).then(res => {
      var user_data = {
        id: item._id,
      ...
      <GridItem xs={12} sm={12} md={12}>
        <Card>
          <CardHeader color="primary">
            <h4 className={classes.cardTitleWhite}>Chat Table</h4>
            ...
            <MaterialTable
                ...
                { 
                    ...
                    render: rowData => (
                      <div>
                        ...
                        {(rowData.admin_join_flg && rowData.admin_id === userData._id) && (
                        ...
                        {(rowData.admin_join_flg && rowData.admin_id !== userData._id) && (
                          rowData.join_flg ?(
                            <Button1  color="danger" className={classes.marginRight}>
              ...
              data={query =>
                new Promise((resolve, reject) => {
                  let url = server_url+'supportchat/support_chat_get_all/'
                  url += query.pageSize
                  url += '/' + (query.page)
                  if(query.search === ""){
                    url += '/0'

...
import {blog_delete} from "Function/Blog.js";
...
export default function TableList(props) {
  ...
  function BlogDelete(id) {
    ...
    blog_delete(data).then(res => {
      store.addNotification({
        content: <MyNotification title="Blog Deleted"/>,
      ...
      <GridItem xs={12} sm={12} md={12}>
        <Card>
          <CardHeader color="primary">
            <h4 className={classes.cardTitleWhite}>Blog Table</h4>
          ...
          <CardBody>
            <div className={classes.buttonPart}>
              <Button variant="contained" color="primary" className={classes.courseCreateButton} onClick={createBlog}>
                Create New Blog
              </Button>
            </div>
            <MaterialTable
              ...
              columns={[
                { 
                  ...
                  render: rowData => (
                    <img
                      ...
                      src={server_url+"blog/img_get/"+rowData.logo}
                      alt="..."
                    />
                  ),
                },
                ...
                { 
                    ...
                    render: rowData => (
                      <div>
                        <Fab color="primary" size="small" aria-label="edit" className={classes.actionButton} onClick={() => BlogView(rowData.id)}>
                            <EditIcon fontSize="small"/>
                        </Fab>
                        <Fab color="secondary" size="small" aria-label="delete" className={classes.actionButton}  onClick={() => BlogDelete(rowData.id)}>
              ...
              data={query =>
                new Promise((resolve, reject) => {
                  let url = server_url+'admin/blog/blog_get_all/'
                  url += query.pageSize
                  url += '/' + (query.page)
                  if(query.search === ""){
                    url += '/0'
